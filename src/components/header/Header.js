...
const Header = () => {
    ...
    const {state:{toggleLan},actions:{handleToggleAr,handleToggleEn}} = useContext(SessionContext);
        ...
        <header className={toggleLan?'header-ar':'header-en'}>
            <nav id="nav_bar">
                <ul className={toggleLan?'row-reverse-ar-style':""}>  
                    {toggleLan ? navAr.map((res)=>
                        <li><a href='#'>{res}</a></li>
                    ):
                    nav.map((res)=><li><a href="#">{res}</a></li>)
                    }
                </ul>
                <a 
                onClick={()=>setToggle(true)} 
                className={toggleLan?'burger-icon-media mg-R remove-mg-L':"burger-icon-media"}
                href="#">
                    <FontAwesomeIcon icon={faBars}/>
                </a>
            </nav>
            <div className={toggleLan?'header row-reverse-ar-style':'header'}>
                <div className={toggleLan?'header-title text-align-end':'header-title'}>
                    <h1 className={toggleLan?'mg-L':''}>{toggleLan?contentAr.title:content.title}</h1>
                    <p>{toggleLan?contentAr.description:content.description}</p>
                    <a href="#" className={toggleLan?'mg-L':''}>{toggleLan?contentAr.button:content.button}</a>
                </div>
            ...
            <div className={toggleLan?"header-lan-Bar to-right-ar":'header-lan-Bar'}>
                <a href="#" onClick={handleToggleAr}>AR</a>
                <a href="#" onClick={handleToggleEn}>EN</a>
            </div>
        </header>
    )
}
